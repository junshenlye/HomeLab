...
def main() -> None:
    ...
    payload = base64.b64encode(Path(args.local).read_bytes()).decode()
    remote = args.remote.replace("'", "'\"'\"'")
    script = f"mkdir -p '$(dirname '{remote}')'\nbase64 -d > '{remote}' <<'B64'\n{payload}\nB64\n"
    response = api(
        env,
        "POST",
    ...
    while time.time() < deadline:
        ...
        if data.get("exited") == 1:
            if data.get("exitcode") != 0:
                raise SystemExit(data.get("err-data", f"guest copy failed: {data.get('exitcode')}"))
            print(f"copied {args.local} -> {args.remote}")
            return
        time.sleep(1)
    raise SystemExit("timed out waiting for guest copy")
